...
public class XmlMD5UserManagerTest {
  ...
  public void testFilePropNotSet() throws Exception {
    ...
    // Should throw
    try {
      final XmlMD5UserManager manager = new XmlMD5UserManager(props);
    } catch (final UndefinedPropertyException e) {
      return;
    }

    fail("XmlMD5UserManager should throw an exception when the file property isn't set");
  }
  ...
  public void testDoNotExist() throws Exception {
    final Props props = new Props(this.baseProps);
    props.put(XmlMD5UserManager.XML_FILE_PARAM, "unit/test-conf/doNotExist.xml");

    try {
      final UserManager manager = new XmlMD5UserManager(props);
    } catch (final RuntimeException e) {
      return;
    }

    fail("XmlMD5UserManager should throw an exception when the file doesn't exist");
  }
  ...
  public void testBasicLoad() throws Exception {
    final Props props = new Props(this.baseProps);
    props.put(XmlMD5UserManager.XML_FILE_PARAM,
        "unit/test-conf/azkaban-MD5users-test1.xml");

    UserManager manager = null;
    try {
      manager = new XmlMD5UserManager(props);
    } catch (final RuntimeException e) {
      e.printStackTrace();
      fail("XmlMD5UserManager should've found file azkaban-users.xml");
    }
    ...
    try {
      ...
      e.printStackTrace();
      fail("XmlMD5UserManager should've returned a user.");
    }
