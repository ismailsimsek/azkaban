...
public class XmlUserManager2Test {
  ...
  public void testFilePropNotSet() throws Exception {
    ...
    try {
      ...
      XmlUserManager2 manager = new XmlUserManager2(props);
    } catch (UndefinedPropertyException e) {
      return;
    }

    fail("XmlUserManager2 should throw an exception when the file property isn't set");
  }
  ...
  public void testDoNotExist() throws Exception {
    Props props = new Props(baseProps);
    props.put(XmlUserManager2.XML_FILE_PARAM, "unit/test-conf/doNotExist.xml");

    try {
      @SuppressWarnings("unused")
      UserManager manager = new XmlUserManager2(props);
    } catch (RuntimeException e) {
      return;
    }

    fail("XmlUserManager2 should throw an exception when the file doesn't exist");
  }

  @Ignore @Test
  public void testBasicLoad() throws Exception {
    Props props = new Props(baseProps);
    props.put(XmlUserManager2.XML_FILE_PARAM,
        "unit/test-conf/azkaban-users-test1.xml");

    UserManager manager = null;
    try {
      manager = new XmlUserManager2(props);
    } catch (RuntimeException e) {
      e.printStackTrace();
      fail("XmlUserManager2 should've found file azkaban-users.xml");
    }
    ...
    try {
      ...
      e.printStackTrace();
      fail("XmlUserManager2 should've returned a user.");
    }
